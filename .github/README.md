...
- Aggregate (struct/union) **by-value** argument and return — struct layout is
  correct, but the psABI eightbyte/HFA classification is not in the CC tables;
  Clang falls back to `DefaultABIInfo` (scalars + indirect). Untested — Lua
  passes aggregates by pointer
